...
int main(int argc, char *argv[]) { 
  ...
  if (new_file) {
    ...
    int dbhd = create_db_header(&header);
    if (dbhd == STATUS_ERROR) {
      printf("Unable to create_db_header\n");
      return EXIT_FAILURE;
    }
  } else {
    ...
    int validation = validate_db_header(dbfd, &header);
    if (validation == STATUS_ERROR) {
